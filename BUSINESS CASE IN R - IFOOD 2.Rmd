...
dataset4 <- cbind(dt_dummied, skewed, subset2[,18:24])
...
s3 <- plot(subset2[,9], subset2[,3], col=cluster$cluster, xlab = 'Gastos com carne nos últimos 2 anos', ylab = 'Renda anual familiar do consumidor')

s4 <- plot(subset2[,10], subset2[,3], col=cluster$cluster, xlab = 'Gastos com peixe nos últimos 2 anos', ylab = 'Renda anual familiar do consumidor')

s5 <- plot(subset2[,11], subset2[,3], col=cluster$cluster, xlab = 'Gastos com doces nos últimos 2 anos', ylab = 'Renda anual familiar do consumidor')

s6 <- plot(subset2[,12], subset2[,3], col=cluster$cluster, xlab = 'Gastos com produtos premium nos últimos 2 anos', ylab = 'Renda anual familiar do consumidor')
...
# Na verdade isto se aplica a qualquer montante gasto, seja vinho, carne, peixe, doce, pois a renda familiar do consumidor está diretamente proporcional ao poder de consumo destes bens.
...
plot(subset2[,17], subset2[,3], col=cluster$cluster, xlab = 'Número de visitas ao site da empresa no último mês', ylab = 'Renda anual familiar do consumidor')

```

```{r echo=FALSE}

# Gráfico abaixo demonstra bem a segmentação de consumidores, pois os consumidores que menos gastaram com carne nos últimos 2 anos, foram os mesmos que menos gastaram com vinho no mesmo período, mantendo a ordem da clusterização bem uniforme entre os 3 clusters.

plot(subset2[,7], subset2[,9], col=cluster$cluster, xlab = 'Gastos com vinhos nos últimos 2 anos', ylab = 'Gastos com carne nos últimos 2 anos')
